...
function FilterBar({ month, year, onMonth, onYear, onReset }) {
  return (
    <div className="flex flex-wrap items-center gap-3 mb-6 p-4 bg-surface-container-lowest rounded-xl premium-shadow">
      ...
      <div className={`mb-3 h-8 flex items-center transition-opacity duration-200 ${tooltip ? "opacity-100" : "opacity-0"}`}>
        {tooltip && (
          <div className="bg-primary text-on-primary text-[11px] font-body px-3 py-1.5 rounded-lg">
            ...
            {tooltip.bookings > 0 && <> · {tooltip.bookings} booking{tooltip.bookings !== 1 ? "s" : ""}</>}
            {" · "}
      ...
      <div className="flex gap-1 mt-2 pl-[calc(2rem+8px)]">
        {["J","F","M","A","M","J","J","A","S","O","N","D"].map((m, i) => (
          <div key={i} className="flex-1 text-center">
            <span className={`text-[9px] md:text-[10px] font-body uppercase ${
          ...
          <tbody>
            {rows.map((b) => (
              <tr key={b._id} className="border-b border-outline-variant/40 hover:bg-surface-container-lowest/60 transition">
                <td className="px-3 md:px-4 py-3 font-body text-body-sm text-on-surface whitespace-nowrap">{b.clientName}</td>
                <td className="px-3 md:px-4 py-3 font-body text-body-sm text-on-surface-variant whitespace-nowrap">{b.eventType}</td>
                ...
                <td className="px-3 md:px-4 py-3">
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full border border-outline-variant text-[11px] font-body uppercase tracking-wider text-on-surface-variant hover:border-secondary hover:text-secondary transition whitespace-nowrap"
                  >
...
export default function Dashboard() {
  ...
  const prevRevenue  = prevFiltered.filter((b) => b.status === "Completed").reduce((sum, b) => sum + (b.totalAmount || 0), 0);
  ...
  const chartData = Array.from({ length: 12 }, (_, i) => {
    ...
    const revenue = mb.filter((b) => b.status === "Completed").reduce((sum, b) => sum + (b.totalAmount || 0), 0);
    return { _id: month, revenue, bookings: mb.length };
  });
  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      <main className="flex-1 bg-surface p-4 md:p-8 min-h-screen overflow-x-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-gutter mb-6">
          {[
            { icon: "event",             label: "Bookings",      val: filteredBookings,  trend: bookingTrend, sub: null },
            { icon: "payments",          label: "Revenue",       val: fmt(filteredRevenue), trend: revenueTrend, sub: "Completed only" },
            ...
            <div key={label} className="bg-surface-container-lowest rounded-xl p-4 md:p-6 premium-shadow">
              <div className="flex justify-between items-start mb-3">
                <div className="w-9 h-9 md:w-12 md:h-12 rounded-lg bg-secondary-fixed flex items-center justify-center text-secondary">
                  <span className="material-symbols-outlined text-[18px] md:text-[22px]">{icon}</span>
                </div>
                <Trend value={trend} />
              </div>
              <div className="font-display text-2xl md:text-headline-md text-primary">{val}</div>
              <div className="font-body text-[10px] uppercase text-on-surface-variant tracking-widest mt-1">{label}</div>
              {sub && <div className="font-body text-[9px] text-on-surface-variant/60 mt-0.5">{sub}</div>}
            </div>
          ))}
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3">
            {[
              ...
              <div key={s} className={`flex flex-col px-3 md:px-5 py-2 md:py-3 rounded-2xl gap-1 ${cls}`}>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] md:text-[11px] font-bold uppercase tracking-wider">{s}</span>
                  <span className="font-display text-base md:text-lg font-bold">{statusCount(s)}</span>
                </div>
                {statusRevenue(s) > 0 && (
                  <span className="text-[10px] font-body opacity-80">{fmt(statusRevenue(s))}</span>
                )}
              </div>
            ))}
          </div>
